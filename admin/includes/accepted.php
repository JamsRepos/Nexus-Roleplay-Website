...
<div class="table-responsive">
    <table class="table">
        ...
        <tbody id="links">
            <?php
            $status = "accepted";
            $stmt = $conn->prepare('SELECT * FROM nexus_apps WHERE status=? ORDER BY id DESC');
            $stmt->execute([$status]);
            while ($row = $stmt->fetch()) {
                ...
                echo "
                                                    ...
                                                    <td class=\"text-center\"><button type=\"button\" class=\"btn btn-sm btn-info\" onclick=\"location.href = 'viewapp.php?id=".$row['id']."'\">VIEW</button></td>
                                                    ";
                echo "</tr>";
            }
            ?>
